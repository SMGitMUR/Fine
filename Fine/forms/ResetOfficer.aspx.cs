...
{
    public partial class ResetOfficer : System.Web.UI.Page
    {
        ...
        protected void btnreset_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "select * from police where po_email=@email and po_num=@ponumber";
            //create three parameterized query for the above select statement 
            cmd.Parameters.AddWithValue("@email", txbfemail.Text);
            cmd.Parameters.AddWithValue("@ponumber", txtuser.Text);
            //Create DataReader         
        ...
        private bool SaveNewPass()
        {
            ...
            SqlCommand cmd2 = new SqlCommand();
            cmd2.Connection = con2;
            cmd2.CommandText = "UPDATE police SET po_pass = @pwd WHERE po_email = @email and po_num = @ponumber";
            cmd2.Parameters.AddWithValue("@pwd", ecrypt.Encrypt(txbfcpass.Text));
            cmd2.Parameters.AddWithValue("@email", txbfemail.Text);
            cmd2.Parameters.AddWithValue("@ponumber", txtuser.Text);
            con2.Open();
            validInsert = cmd2.ExecuteNonQuery() > 0;
            if (!validInsert)
            {
